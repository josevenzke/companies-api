...
def test_create_company_with_not_numeric_cnpj(client) -> None:
    response = client.post(companies_url,data={"name":"Amazon","cnpj":"128asdf910111a"})
    assert response.status_code == 400
    assert json.loads(response.content) == {'cnpj': ['CNPJ value should include only numbers']}

@pytest.mark.django_db
def test_create_company(client) -> None:
    response = client.post(companies_url,data={"name":"Amazon","cnpj":"12345678910111"})
    response_content = json.loads(response.content)
...
def test_patch_one_company_employees_list(client) -> None:
    ...
    response = client.patch(companies_url+f'{test_company.id}/',data=json.dumps({'employees': [test_employee.id]}), content_type='application/json')
    response_content = json.loads(response.content)
    assert response.status_code == 200
    assert response_content.get("name") == test_company.name
    assert response_content.get("cnpj") == test_company.cnpj
...
def test_patch_one_company_withou_employee_id(client) -> None:
    test_company = Company.objects.create(name="Amazon",cnpj='12345678910111')
    response = client.patch(companies_url+f'{test_company.id}/', content_type='application/json')
    assert response.status_code == 400
